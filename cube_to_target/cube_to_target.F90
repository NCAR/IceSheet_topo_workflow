...
!!++jtb 11/21/17 do not fix global mean
#ifdef GREENLAND
      write(*,*) "  Skipping volume correction in GREENLAND config .... "
#else
...
!      output_fname = './output/'//trim(output_grid)//trim(ofile$)//'_'//date$//'.nc' !++ml
      output_fname = './output/'//trim(output_grid)//trim(ofile$)//'.nc'
...
!  integer, dimension(2) :: nc_lat_vid,nc_lon_vid
  ...
  status = nf_put_var_double (foutid, terrid, terr*9.80616)  !!++ml
!  status = nf_put_var_double (foutid, terrid, terr*9.80616/6.) !!++ml
  if (status .ne. NF_NOERR) call handle_err(status)
